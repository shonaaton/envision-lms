...
export async function notifyUser(
  ...
  const recipient: { email?: string; name?: string; phone?: string; countryCode?: string; role?: string } | null = metadata?.email
    ? { email: String(metadata.email), name: String(metadata.recipientName || ""), phone: String(metadata.phone || ""), countryCode: String(metadata.countryCode || "") }
    : await User.findById(user).select("email name phone countryCode role").lean<{ email?: string; name?: string; phone?: string; countryCode?: string; role?: string } | null>();
  if (recipient?.email) {
  ...
  await sendWhatsAppAutomationTemplate({
    user: {
      ...
      phone: recipient?.phone || metadata?.phone || "",
      countryCode: recipient?.countryCode || metadata?.countryCode || "",
    },
