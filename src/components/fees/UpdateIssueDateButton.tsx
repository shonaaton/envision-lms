        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 px-4 py-6">
          <form action={action} className="w-full max-w-md rounded-lg bg-white p-4 shadow-2xl">
            ...
            <div className="mb-4 flex items-start justify-between gap-3">
              <div>
                <h3 className="text-base font-bold text-slate-950">Update Issue Date</h3>
                <p className="mt-1 text-sm text-slate-500">Choose the invoice issue date to show on this record and PDF.</p>
              </div>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button
                type="button"
                onClick={() => setIssueDate(dateInputValue(dueDate))}
            ...
            <div className="mt-4 flex flex-wrap justify-end gap-2">
              ...
              <button disabled={!issueDate} className="inline-flex h-10 items-center gap-2 rounded-md bg-sky-700 px-4 text-sm font-bold text-white disabled:cursor-not-allowed disabled:bg-slate-300">
                <CalendarClock size={15} /> Save Date
              </button>
            </div>
          </form>
        </div>
