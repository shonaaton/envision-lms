  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#12031a] text-slate-950 lg:h-screen lg:overflow-hidden">
      ...
      <div className="relative mx-auto grid min-h-screen max-w-7xl gap-5 px-4 py-5 sm:px-6 lg:h-screen lg:grid-cols-[minmax(330px,0.85fr)_minmax(620px,1.15fr)] lg:px-8">
        ...
        {/*
          The one way out of the funnel. This page is indexed and is linked from
          most of the site, but it used to link nowhere except the login screen,
          so everything pointing at it terminated here. Kept to a plain text row
          rather than a card wall: the form is still the point of the page.
          Written out inline rather than read from `internalLinks` so the course
          and centre data does not get pulled into this client bundle.
        */}
        <nav aria-label="Explore the academy" className="mt-4 shrink-0 rounded-lg border border-white/12 bg-white/[0.06] p-4 text-center text-white">
          <p className="text-[11px] font-black uppercase tracking-[0.14em] text-accent">Not ready to sign up?</p>
          <ul className="mt-2 flex flex-wrap items-center justify-center gap-x-4 gap-y-1.5 text-xs font-semibold text-white/75">
            {[
              ["Online Chess Coaching Courses", "/online-chess-coaching-courses"],
              ["Chess Academy in Kolkata", "/chess-academy-in-kolkata"],
              ["Student Success Stories", "/success-stories"],
              ["Contact Us", "/contact-us"],
            ].map(([label, href]) => (
              <li key={href}>
                <Link href={href} className="hover:text-accent hover:underline">{label}</Link>
              </li>
            ))}
          </ul>
        </nav>
