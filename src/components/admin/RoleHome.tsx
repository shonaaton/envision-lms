...
export default async function RoleHome({ user }: { user: SessionUser & { name?: string | null } }) {
  ...
  const salesStaff = user.id ? await isSalesStaff(user.id) : false;
  ...
  return <div className="space-y-5">
    ...
    {user.id && (salesStaff || !["admin", "sub-admin"].includes(String((user as any).role))
      ? <LeadOwnerDemosPanel userId={user.id} scope="mine" showEmpty={salesStaff} />
      : <LeadOwnerDemosPanel userId={user.id} scope="all" />)}
