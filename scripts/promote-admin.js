 ...
 *    - Open your cluster → Browse Collections → envision_chess.users
...
if (!email || !uri) {
  console.error("Usage: MONGODB_URI=... node scripts/promote-admin.js <email>");
  process.exit(1);
}
...
try {
  ...
  else console.log(`✓ ${email} is now admin`);
} finally {
  await client.close();
}
