...
export default function PgnLibraryPage() {
  ...
  const load = useCallback(async () => {
    ...
    const gamesRequest = fetch("/api/pgn?limit=5000&summary=1", { cache: "no-store" })
      .then(async (response) => {
    ...
    const foldersRequest = fetch("/api/pgn/folders", { cache: "no-store" })
      .then(async (response) => {
        if (!response.ok) return;
        const docs = await response.json();
    ...
    await Promise.allSettled([gamesRequest, foldersRequest]);
  }, [role]);
  ...
  return (
    <div className="-m-6 min-h-screen space-y-6 bg-slate-50 p-6 text-slate-950">
      ...
      <section className="min-h-[376px] rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
        ...
        {gamesLoading && foldersLoading && !games.length && !folders.length ? (
          <LibraryLoadingState />
        ) : currentFolder ? (
...
function LibraryLoadingState() {
  return (
    <div className="space-y-8" aria-live="polite" aria-label="Loading PGN library">
      {["Shared Library", "Personal Library"].map((section) => (
        <section key={section} className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {Array.from({ length: 3 }).map((_, index) => (
              <div key={index} className="h-36 animate-pulse rounded-md border border-slate-200 bg-white p-4 shadow-sm">
