    ...
    <div className="min-h-screen bg-slate-50 px-4 py-4 text-slate-950 sm:px-6 lg:px-8">
      <div className="mb-4 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-brand/70">Credit History</p>
            <h1 className="text-xl font-bold text-slate-950">Credit Usage History</h1>
          </div>
          <Link
            href="/fees"
            className="inline-flex h-10 items-center justify-center gap-2 rounded-lg border border-brand/15 bg-white px-4 text-sm font-bold text-brand hover:bg-brand hover:text-white"
          >
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-base font-bold text-slate-950">All Credit Transactions</h2>
            <p className="text-xs text-slate-500">Credits added, used, and adjusted.</p>
          </div>
          <span className="w-fit rounded-full bg-brand/10 px-3 py-1 text-xs font-bold text-brand">{ledgers.length} entries</span>
        </div>

        {ledgers.length === 0 ? (
          <div className="rounded-lg border border-dashed border-slate-300 bg-slate-50 p-6 text-center">
            <h3 className="text-sm font-bold text-slate-950">No credit history yet</h3>
            <p className="mt-1 text-xs text-slate-500">Credit purchases and class deductions will appear here.</p>
          </div>
        ) : (
          <>
                  ...
                  {ledgers.map((item: any) => {
                    ...
                    return (
                      <tr key={item._id.toString()} className="border-t border-slate-100 align-top">
                        ...
                        <td className="px-4 py-3 font-bold text-emerald-700">{credits > 0 ? `+${credits}` : "-"}</td>
                        <td className="px-4 py-3 font-bold text-rose-700">{credits < 0 ? credits : "-"}</td>
                        <td className="px-4 py-3 font-bold text-slate-950">{item.balanceAfter}</td>
            ...
            <div className="space-y-3 md:hidden">
              {ledgers.map((item: any) => {
                const credits = Number(item.credits || 0);
                return (
                  <article key={item._id.toString()} className="rounded-lg border border-slate-200 bg-slate-50 p-3 shadow-sm">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className="font-bold text-slate-950">{transactionLabel(item.type, credits)}</p>
                      ...
                      <span className={`shrink-0 rounded-lg px-3 py-2 text-base font-bold ${credits >= 0 ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-700"}`}>
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-2 text-sm">
                      <div className="rounded-lg bg-white p-3">
                        ...
                        <p className="mt-1 font-bold text-emerald-700">{credits > 0 ? `+${credits}` : "-"}</p>
                      </div>
                      <div className="rounded-lg bg-white p-3">
                        <p className="text-xs font-bold uppercase tracking-[0.12em] text-slate-400">Deducted</p>
                        <p className="mt-1 font-bold text-rose-700">{credits < 0 ? credits : "-"}</p>
                      </div>
                      <div className="rounded-lg bg-white p-3">
                        <p className="text-xs font-bold uppercase tracking-[0.12em] text-slate-400">Balance</p>
                        <p className="mt-1 font-bold text-slate-950">{item.balanceAfter}</p>
                      </div>
                      <div className="rounded-lg bg-white p-3">
